...
# 3. Cache Management / Recent Interaction Utilities
hx cache [args...]
...
| **`hx set-model [args]`** | **Set Session Model:** Quickly updates your current session's `$HX_MODEL` environment variable using a specific model string or configuration. | Calls `model.sh`. |
...
### 3. Cache & Interaction Utilities (`hx cache`, `why`, etc.)
The framework uses local JSON files for caching and provides utilities to inspect these cached interactions immediately.

**Cache Management:**
* **`hx cache [args]`**: Manage storage settings (e.g., `clear`, `show`, `enable/disable`).

**Interaction Inspection (Operates on the most recent interaction):**
If a local cache exists, these commands extract specific components from your latest inference:
