...
A safety gate for pipelines that forwards piped data to stdout only after explicit user confirmation. It is designed to prevent the automatic execution of potentially destructive commands generated by an LLM.

#### Synopsis

```
<input-data> | pipetest [PROMPT...]
```
...
4.  **Decision:**
    ...
    *   Otherwise, it prints `🚫 discarded` to `stderr` and exits with code `1`.

#### Input

*   **stdin:** Any data (plain text, JSON conversation arrays, etc.) to be evaluated.
*   **Arguments:** An optional string used as a descriptive prompt for the user during the confirmation step.

#### Output

| Condition | Destination | Content |
|-----------|-------------|---------|
| **Confirmed** (`y/Y`) | `stdout` | The exact content originally received on `stdin`. |
| **Rejected** (else) | `stderr` | `🚫 discarded` |
